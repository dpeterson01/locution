...
This guide covers how to set up the development environment and build Locution from source across different platforms.
...
```bash
git clone git@github.com:dpeterson01/locution.git
cd locution
```
...
The raw binary (`src-tauri/target/release/Locution`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/locution/src-tauri/target/release/bundle/deb/Locution_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/Locution /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Locution.desktop /usr/share/applications/
sudo ldconfig
```
...
```bash
sudo cp src-tauri/target/release/Locution /usr/bin/
...
```
Bundling Locution_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Locution.AppDir --plugin gtk --output appimage
```
...
this because it builds from a short root such as `D:\a\locution`.)
...
Alternatively, clone the repo to a short root (e.g. `C:\locution`).
