  ...
  ({ descriptionMode = "tooltip", grouped = false }) => {
    ...
    return (
      <>
        ...
        {selectedStyle !== "none" && (
          <ToggleSwitch
            checked={getSetting("overlay_always_show") ?? true}
            onChange={(enabled) =>
              updateSetting("overlay_always_show", enabled)
            }
            isUpdating={isUpdating("overlay_always_show")}
            label={t("settings.advanced.overlay.alwaysShow.title")}
            description={t("settings.advanced.overlay.alwaysShow.description")}
            descriptionMode={descriptionMode}
            grouped={grouped}
          />
        )}
