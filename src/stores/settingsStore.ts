...
const settingUpdaters: {
  ...
  play_sound_on_end: (value) =>
    commands.changePlaySoundOnEndSetting(value as boolean),
  ...
  adaptive_cleanup: (value) =>
    commands.changeAdaptiveCleanupSetting(value as boolean),
  context_capture_enabled: (value) =>
    commands.changeContextCaptureEnabledSetting(value as boolean),
  short_threshold_chars: (value) =>
    commands.changeShortThresholdCharsSetting(value as number),
  short_model: (value) => commands.changeShortModelSetting(value as string),
  long_model: (value) => commands.changeLongModelSetting(value as string),
  skip_llm_under_chars: (value) =>
    commands.changeSkipLlmUnderCharsSetting(value as number),
  ...
  snippets: (value) => commands.updateSnippetsSetting(value as Snippet[]),
  style_card: (value) => commands.updateStyleCardSetting(value as string),
  style_card_enabled: (value) =>
    commands.changeStyleCardEnabledSetting(value as boolean),
  per_app_auto_mode_enabled: (value) =>
    commands.changePerAppAutoModeEnabledSetting(value as boolean),
  per_app_mode_map: (value) =>
    commands.updatePerAppModeMapSetting(
      value as Partial<{ [key in string]: string }>,
    ),
  // default_mode_id intentionally has no entry here — it only changes via
  // setPostProcessSelectedPrompt (the shared manual mode-switch path) or the
  // per-app rule engine in Rust, never a generic updateSetting call.
};
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    initialize: async () => {
      ...
      // Backend-initiated settings writes (tray mode switch, cycle-mode
      // hotkey, overlay mode popover) broadcast "settings-changed"; refresh
      // so every window stays in sync. Read-only here — never write back.
