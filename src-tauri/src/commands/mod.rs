...
#[specta::specta]
#[tauri::command]
pub async fn probe_ollama_setup() -> crate::ollama_setup::OllamaProbeResult {
    crate::ollama_setup::probe_ollama().await
}

/// Install Ollama (if absent) and ensure the service is running. Fails soft
/// with a typed error the wizard maps onto its failure-matrix UI; the wizard
...
/// Pull one Ollama model, streaming progress via the `"ollama-pull-progress"`
/// event. Idempotent — safe to re-call after an interrupted pull.
#[specta::specta]
#[tauri::command]
pub async fn pull_ollama_model(
    ...
    crate::ollama_setup::pull_model(&app, &model_name).await
}
