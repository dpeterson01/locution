...
/// Filter shared by the persisted (file + webview-mirrored) log targets.
///
/// `enigo` logs a DEBUG line on every call to `.location()`, and the
/// recording overlay polls that at ~12Hz whenever it's visible — chatty
/// enough to fill the 500KB file-log cap within minutes of normal use,
/// rotating away earlier lines (including diagnostics::record_failure's
/// DIAG_EVENT lines) before anyone gets a chance to read or export them.
/// Cap `enigo` to Warn+ in these two targets regardless of the app's log
/// level; the console target (RUST_LOG-driven) is untouched, so a developer
/// can still opt into full enigo tracing there.
fn passes_persisted_log_filter(metadata: &log::Metadata, file_level: u8) -> bool {
    if metadata.target().starts_with("enigo") && metadata.level() > log::Level::Warn {
        return false;
    }
...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    let tray = TrayIconBuilder::new()
        ...
        .on_menu_event(|app, event| match event.id.as_ref() {
            ...
            id if id.starts_with("mode_select:") => {
                let prompt_id = id.strip_prefix("mode_select:").unwrap().to_string();
                if let Err(e) =
                    shortcut::set_post_process_selected_prompt(app.clone(), prompt_id.clone())
                {
                    log::error!("Failed to switch mode via tray: {}", e);
                } else {
                    log::info!("Mode switched to {} via tray.", prompt_id);
                }
            }
    ...
    // Create the recording overlay window (hidden by default). The idle dash
    // pill is surfaced when the overlay webview reports ready
...
pub fn run(cli_args: CliArgs) {
    ...
    let specta_builder = Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            ...
            shortcut::change_play_sound_on_end_setting,
            ...
            shortcut::resize_overlay,
            overlay::overlay_cursor_position,
            overlay::overlay_ready,
            shortcut::change_context_capture_enabled_setting,
            shortcut::change_adaptive_cleanup_setting,
            shortcut::change_short_threshold_chars_setting,
            ...
            shortcut::set_ollama_setup_status,
        ...
        .plugin(
            LogBuilder::new()
                ...
                .targets([
                    ...
                    .filter(|metadata| {
                        passes_persisted_log_filter(
                            metadata,
                            FILE_LOG_LEVEL.load(Ordering::Relaxed),
                        )
                    }),
    ...
    // Single-instance forwards CLI args to an already-running Locution and exits.
