...
/// Expands phrase triggers into their configured expansion text, verbatim.
///
/// Applied to the final pasted text (see `actions.rs::process_transcription_output`),
/// after any LLM cleanup — cleanup would otherwise be free to reword an expanded
/// snippet like a signature block or boilerplate reply.
///
/// # Arguments
/// * `text` - The text to expand snippets in
/// * `snippets` - `(trigger, expansion)` pairs; empty/whitespace-only triggers are skipped
///
/// # Returns
/// The text with every case-insensitive, whole-word/phrase trigger match replaced by its
/// expansion, inserted literally (a `$` in the expansion must never be read as a regex
...
mod tests {
    ...
    #[test]
    fn test_apply_snippets_basic_match() {
        let text = "please send my email now";
        let snippets = vec![("my email".to_string(), "derek@example.com".to_string())];
        let result = apply_snippets(text, &snippets);
        assert_eq!(result, "please send derek@example.com now");
    }

    #[test]
    fn test_apply_snippets_case_insensitive() {
        let text = "MY EMAIL please";
        let snippets = vec![("my email".to_string(), "derek@example.com".to_string())];
        let result = apply_snippets(text, &snippets);
        assert_eq!(result, "derek@example.com please");
    }

    #[test]
    fn test_apply_snippets_multi_word_trigger() {
        let text = "here is my home address for the form";
        let snippets = vec![(
            "my home address".to_string(),
            "123 Main St, Springfield".to_string(),
        )];
        let result = apply_snippets(text, &snippets);
        assert_eq!(result, "here is 123 Main St, Springfield for the form");
    }

    #[test]
    fn test_apply_snippets_empty_list_is_noop() {
        let text = "nothing should change here";
        let result = apply_snippets(text, &[]);
        assert_eq!(result, text);
    }

    #[test]
    fn test_apply_snippets_blank_trigger_skipped() {
        let text = "leave this alone";
        let snippets = vec![("   ".to_string(), "should not appear".to_string())];
        let result = apply_snippets(text, &snippets);
        assert_eq!(result, text);
    }

    #[test]
    fn test_apply_snippets_expansion_dollar_sign_not_expanded() {
        // Regression guard: Regex::replace_all treats `$` in the replacement as a
        // capture-group reference unless wrapped in NoExpand. A price/template-var
        // expansion must survive literally.
        let text = "my rate is what I charge";
        let snippets = vec![("my rate".to_string(), "$150/hr".to_string())];
        let result = apply_snippets(text, &snippets);
        assert_eq!(result, "$150/hr is what I charge");
    }

    #[test]
    fn test_apply_snippets_multiple_occurrences() {
        let text = "my sig then more text then my sig again";
        let snippets = vec![("my sig".to_string(), "Best, Derek".to_string())];
        let result = apply_snippets(text, &snippets);
        assert_eq!(result, "Best, Derek then more text then Best, Derek again");
    }
