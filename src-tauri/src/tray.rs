...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/locution.png",
...
/// Best-effort current tray state for menu rebuilds that happen outside an
/// icon-state transition (e.g. a mode switch): a rebuild mid-recording must
/// keep the recording menu (with its Cancel item) rather than reverting to
/// the idle variant.
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Locution v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Locution v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    // Build mode submenu — label is the active cleanup mode name
    let selected_mode_id = settings.post_process_selected_prompt_id.clone();
    let mode_submenu_label = settings
        .post_process_prompts
        .iter()
        .find(|p| Some(&p.id) == selected_mode_id.as_ref())
        .map(|p| p.name.clone())
        .unwrap_or_else(|| strings.mode.clone());

    let mode_submenu = {
        let submenu = Submenu::with_id(app, "mode_submenu", &mode_submenu_label, true)
            .expect("failed to create mode submenu");

        for prompt in &settings.post_process_prompts {
            let is_active = Some(&prompt.id) == selected_mode_id.as_ref();
            let item_id = format!("mode_select:{}", prompt.id);
            let item =
