...
fn build_apple_intelligence_bridge() {
    ...
    // The framework header/dylib can be present under Xcode Command Line Tools alone,
    // but the @Generable macro needs the macro-expansion plugin that only ships with
    // the full Xcode.app toolchain — so also require that before attempting the real
    // source file, or swiftc fails with "external macro implementation type ... could
    // not be found". Fall back to stubs otherwise (this project never selects the
    // Apple Intelligence post-process provider anyway).
    let framework_path =
        Path::new(&sdk_path).join("System/Library/Frameworks/FoundationModels.framework");
    let has_full_xcode = Command::new("xcode-select")
        .arg("-p")
        .output()
        .map(|o| String::from_utf8_lossy(&o.stdout).contains("Xcode.app"))
        .unwrap_or(false);
